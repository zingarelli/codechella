...
Site do CodeChella: um festival de música alternativa para Devs!
...
Este projeto é o resultado da minha participação na 6ª edição do Desafio de Front End da Alura. Por meio de uma abordagem de aprendizado baseado em desafios, nós colocamos nossos conhecimentos em ação resolvendo um desafio simulado em um projeto do mundo real, que incluiu receber tasks via Trello e lidar com prazos para entrega. 

O desafio foi proposto pela [**Alura**](https://www.alura.com.br), uma escola online de tecnologia, que disponibilizou os cards Trello semanalmente, bem como o layout das telas no Figma (o que também incluiu a tipologia e paleta de cores).

Existe toda uma equipe da Alura por trás do desafio, sendo nosso contato maior com a [**Mônica Hillman**](https://www.linkedin.com/in/monicamhillman/), instrutora de Front End, o [**Lucas de Freitas Pereira**](https://br.linkedin.com/in/lucas-de-freitas-pereira-b89044192), que cuida das comunidades no Discord da Alura, e o [**Vinícius Morais**](https://www.linkedin.com/in/vinícius-morais/), coordenando e dando suporte nas lives semanais. Meu muito obrigado a todos.
...
Criei um componente `<ScrollToTop />` para que as páginas sejam sempre mostradas a partir do topo na janela do navegador. Copiei o código desse componente do [tutorial disponibilizado pelo próprio React Router](https://v5.reactrouter.com/web/guides/scroll-restoration/scroll-to-top).
...
A cada semana é disponibilizado um Trello com as tarefas a serem realizadas, simulando um "sprint" de um processo de metodologia ágil.
...
O site possui dois temas de cores: Verão (com cores mais quentes) e Boreal (com cores mais frias). Foi adicionado ao final do rodapé da tela dois botões para que a pessoa escolha o tema que preferir. A alteração ocorre dinamicamente e a escolha fica salva no *localstorage* do navegador. O gif abaixo mostra a página inicial e os dois temas sendo aplicados: 
...
Recebe via props uma imagem, uma posição para esta imagem (opcional) e um texto (também opcional). Quando o texto está presente, é aplicada uma opacidade à imagem para que o texto fique destacado. 

O banner é preenchido com a imagem por meio da propriedade CSS `background`, por padrão centralizada. Esse posicionamento pode ser alterado por meio da prop `position`.

#### Button

Renderiza um botão, com um texto e uma imagem, recebidos via props. A imagem é um ícone decorativo e opcional.

#### Header

Renderiza o logo e o menu de navegação. Na versão mobile, o menu é substituído por um "menu hambúrguer", que revela os links de navegação quando é clicado, como pode ser visto no gif abaixo: 
...
Este foi um **componente que me deu muito trabalho**, pois ele é reutilizado em várias seções das telas, porém com posicionamentos, margens e alinhamentos de texto variados. Segue abaixo uma imagem mostrando vários artigos em diferentes estilos: 
...
Refiz diversas vezes este componente, tentando torná-lo o mais genérico possível para englobar todas essas mudanças de estilo via props, e isso virou uma dor de cabeça... No final, decidi somente alterar a posição da imagem por meio de uma prop adicional: `imagePosition`. A imagem sempre fica no topo em telas mobile e tablet. Já para telas desktop, o posicionamento é feito por meio dessa prop (à esquerda do texto, por padrão). 
...
A página principal mostra uma lista com as bandas de cada dia do evento, o que foi denominado *"lineup"*. As bandas são mostradas com certa "hierarquia", recebendo tamanho e peso de fontes diferentes. Para fazer essa renderização do lineup, decidi criar três componentes, para melhor trabalhar com a estilização:
...
O componente `<AppRoutes />` é o principal da aplicação, sendo o responsável por gerenciar as rotas e respectivas páginas.
...
Além das três páginas solicitadas, também criei uma página padrão para rotas inexistentes (a famosa "página 404"). Ela mostra uma mensagem de erro e uma foto de gatinho (por que não? 🙃) e pode ser conferida [clicando aqui](https://codechella-puce.vercel.app/errrrrro).
...
Por enquanto, o conteúdo do ingresso está estático, servindo apenas de exemplo sobre como ele deve ficar. No próximo sprint, o conteúdo deverá ser preenchido com os dados recebidos do formulário. Segue abaixo a imagem de um ingresso preenchido.
...
A terceira semana foi dedicada às interações da pessoa usuária com as páginas do site, além de retoques e mudanças ao que já foi desenvolvido. 

As seções a seguir são um resumo das atividades realizadas:
...
Também adicionei uma animação ao ícone de flecha que fica ao lado da pergunta: ela rotaciona quando o componente "abre" ou "fecha", como forma de dica visual à pesssoa usuária. Essa rotação foi feita por meio da adição/remoção de uma classe aplicada ao pseudo-elemento `::after`.

Segue abaixo um gif mostrando um componente `<Accordion />` abrindo e fechando conforme é clicado no seu título.
...
#### Componente para mudança de tema

Anteriormente, a mudança de tema era feita no componente `<Footer />`. Resolvi tirar essa responsabilidade do `<Footer />` e criei um componente `<ThemeSwitch />` para renderizar e lidar com a mudança do tema. É uma mudança pequena, mas acho que deixa o código mais bem organizado, tentando aplicar o *"Single-responsibility Principle"* do padrão SOLID.

### Semana 4

Com o site terminado e funcional, a última semana foi dedicada a **ir além** do que foi solicitado no desafio. Desse modo, **me desafiei a aplicar testes unitários ao projeto**. Esse é um tema que ainda não domino e que estudei somente o básico, então este foi um ótimo desafio (apesar de ter dado bastante dor de cabeça e momentos diários de frustração😔).

Para aplicar os testes, utilizei o Jest e a React Testing Library (RTL). Ambos são instalados por padrão quando se cria uma aplicação utilizando o Create React App. O curso da Alura sobre [Testes em React](https://www.alura.com.br/curso-online-react-js-testes-automatizados-front-end) ministrado pelo [Neilton Seguins](https://www.linkedin.com/in/neílton-seguins/) foi de grande importância para eu dar meu primeiro mergulho no mundo dos testes. Além do curso, fiz muito uso da [documentação do Jest](https://jestjs.io/docs/getting-started) e também da [documentação da Testing Library](https://testing-library.com/docs/), além, é claro, do Google e do Stack Overflow.

Tive bastante dificuldade em entender o que deveria ser testado (e como o teste deveria ser feito). Algumas coisas que eu gostaria de testar, como funções internas, acabavam não dando certo, pois o escopo da RTL é testar a interface pensando em ações da pessoa usuária ao interagir com o site. Decidi por **aplicar testes unitários aos componentes criados**, cobrindo aspectos mais simples, como verificar se os elementos são renderizados (ou não são, dependendo de alguma variável de estado) e se funções passadas via props são ativadas após algum evento do usuário.

Uma coisa legal que consegui testar foi a mudança de tema, verificando se o localStorage era atualizado com o tema escolhido a cada ação de clique no tema desejado.

Outro teste que deu muito trabalho (mas também muito orgulho ao funcionar) foi criar testes para o componente de formulário. Consegui verificar se os campos renderizavam, se as validações estavam sendo aplicadas, bem como se a submissão era feita somente após passar pelas validações. Esse teste também me mostrou que o componente está muito complexo e cheio de responsabilidades, e deve ser refatorado em componentes menores. Mas isso fica para uma próxima. Por hora, encerro o desafio aqui, **exausto, mas satisfeito com o resultado e minha evolução no Front End**.
...
There's a whole team of Alura's staff behind this challenge, but our main point of contact was [**Mônica Hillman**](https://www.linkedin.com/in/monicamhillman/), Front End instructor, [**Lucas de Freitas Pereira**](https://br.linkedin.com/in/lucas-de-freitas-pereira-b89044192), community manager on Discord, and [**Vinícius Morais**](https://www.linkedin.com/in/vinícius-morais/) coordinating and supporting the weekly livestreams. My sincere thanks to everyone.
...
Other libraries and dependencies can be checked in the `package.json` file.
...
I've continued developing the rest of the pages: FAQ page, form page and ticket page. For now, I've focused on the **visual aspect of the pages**, including their content and styles. User interaction with elements of the page, as well as how the data in the form will be shared with other pages, will be developed in the following weeks.
...
With a fully functional website, I've decided to **tackle a personal challenge during the last week: unit tests**. This is something I only know the basics, so it was a great, albeit painful, challenge.

I've used Jest and the React Testing Library (RTL) to develop the tests. Both come installed by default when using the Create React App. 

In order to take my first steps in the world of testing React apps, I've taken a [course at Alura](https://www.alura.com.br/curso-online-react-js-testes-automatizados-front-end), taught by [Neilton Seguins](https://www.linkedin.com/in/neílton-seguins/). I've also used [Jest documentation](https://jestjs.io/docs/getting-started) and [Testing Library documentation](https://testing-library.com/docs/) a lot, besides, of course, Google and Stack Overflow.

There's still a lot I need to learn about unit testing using Jest and RTL (and tests in general). Even though I'm really exhausted, I'm happy with what I achieved in only one week. With that in mind, I end my participation in this challenge.